...
export default function ADF001() {
  const [messages, setMessages] = useState([
    { role: 'assistant', content: '欢迎来到 ADF-001 创意孵化中心 v1.8\n我是你的 AI 导演。\n\n告诉我一句话你的创意，我会帮你孵化项目并进行专业角色设计。' }
  ]);
...
下一步：点击右侧按钮进入角色设计。`;
  ...
  const generateImagePrompt = (role: any) => {
    const prompt = `一个${role.age}岁亚洲${role.roleType.includes('主角') || role.roleType.includes('男') ? '男性' : '女性'}，名叫${role.name}，${role.appearance}，${role.personality}的表情，废土电影风格，高细节，电影光影，8k --ar 3:4`;
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-xl text-zinc-400 mb-10 text-center">AI 影视导演 · 创意孵化中心 v1.8</p>
        ...
        <div className="grid grid-cols-12 gap-6 h-[calc(100vh-140px)]">
          ...
          <div className="col-span-7 bg-zinc-900 rounded-3xl flex flex-col border border-zinc-700 overflow-hidden">
            ...
            <div className="p-6 border-t border-zinc-700 bg-zinc-950">
              <div className="flex gap-4">
                ...
                />
                <button onClick={sendMessage} disabled={isLoading || !input.trim()} className="bg-emerald-500 hover:bg-emerald-600 px-12 rounded-2xl font-medium">发送</button>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-5 bg-zinc-900 rounded-3xl flex flex-col border border-zinc-700 overflow-hidden">
            <div className="p-6 border-b border-zinc-700 bg-zinc-950 flex justify-between items-center">
              ...
              <button onClick={addNewRole} className="bg-zinc-700 hover:bg-zinc-600 px-5 py-2 rounded-xl text-sm">+ 新角色</button>
            </div>

            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              {!showRoleDesign ? (
                <div className="h-full flex items-center justify-center text-zinc-500">
                  <button onClick={startRoleDesign} className="bg-blue-600 hover:bg-blue-700 px-10 py-4 rounded-2xl font-medium">
                    🎭 进入角色设计阶段
                  </button>
                </div>
              ) : (
                roles.map((role, index) => (
                  <div key={index} className="bg-zinc-800 p-6 rounded-2xl">
                    <div className="flex justify-between mb-4">
                      ...
                      <span className="text-xs px-3 py-1 bg-zinc-700 rounded-full">{role.roleType}</span>
                    </div>
                    <div className="space-y-3 text-sm">
                      <div>年龄 <input value={role.age} onChange={(e) => updateRole(index, 'age', e.target.value)} className="bg-zinc-900 w-full px-3 py-1.5 rounded" /></div>
                      <div>身高 <input value={role.height} onChange={(e) => updateRole(index, 'height', e.target.value)} className="bg-zinc-900 w-full px-3 py-1.5 rounded" /></div>
                      <div>外貌 <input value={role.appearance} onChange={(e) => updateRole(index, 'appearance', e.target.value)} className="bg-zinc-900 w-full px-3 py-1.5 rounded" /></div>
                      <div>性格 <input value={role.personality} onChange={(e) => updateRole(index, 'personality', e.target.value)} className="bg-zinc-900 w-full px-3 py-1.5 rounded" /></div>
                    </div>
                    <div className="flex gap-3 mt-6">
                      ...
                      <button onClick={() => alert('角色描述功能开发中')} className="flex-1 bg-zinc-700 hover:bg-zinc-600 py-3 rounded-xl text-sm">角色描述</button>
                    </div>
                  </div>
                ))
              )}
